...
const getCategoryEmoji = (category) => {
  const map = {
    // --- РАСХОДЫ ---
    'Продукты': '🛒',
    'Еда вне дома': '🍔', // Кафе, рестораны, фастфуд
    'Кофе': '☕',
    'Алкоголь': '🍺',
    'Табак': '🚬',
    ...
    'Дом': '🏠', // Аренда
    'ЖКУ': '💡', // Коммуналка, свет, вода
    'Ремонт': '🛠️',
    'Связь': '📱',
    'Интернет': '🌐',

    'Здоровье': '💊',
    'Красота': '💅', // Стрижка, косметика
    'Спорт': '💪',
  ...
  // Если не нашли, пробуем найти "Еда" для общих категорий
  if (category === 'Еда') return '🍔';
  
  return '✨';
};

// --- AI HELPERS (СУПЕР-ПРОМПТ) ---
const analyzeText = async (text, currency = 'UZS') => {
  try {
    ...
    const prompt = `
      ...
      RULES:
      1. "25k" = 25000.
      2. Type: "expense" (spending) or "income" (earning).
      3. Category: Choose STRICTLY from the list below. If unsure, use "Прочее".
      ...
      - "заправка 300" -> {"amount": 300, "category": "Бензин", "type": "expense"}
      - "аптека 50к" -> {"amount": 50000, "category": "Здоровье", "type": "expense"}
      - "зп 10млн" -> {"amount": 10000000, "category": "Зарплата", "type": "income"}
      - "проценты 50к" -> {"amount": 50000, "category": "Вклады", "type": "income"}
      - "шабашка 200" -> {"amount": 200, "category": "Фриланс", "type": "income"}
      - "вернули долг 100" -> {"amount": 100, "category": "Возврат долга", "type": "income"}
...
bot.start(async (ctx) => {
  ...
  try {
    ...
    ctx.reply('Мозг расширен! Я знаю 40+ категорий: бензин, ЖКУ, кэшбэк, фриланс и многое другое.', 
